...
var path    = require('path'),
	...
	SSH2    = require('ssh2'),
	log     = require('./log'),
	cfgSsh  = require(path.join(process.env.CWD, 'config', 'ssh')),
	cfgHttp = require(path.join(process.env.CWD, 'config', 'static')),
...
if ( cfgSsh.active ) {
	...
	if ( command.ssh && (config = cfgSsh.profiles[command.ssh === true ? 'develop' : command.ssh]) ) {
		// init
		conn = new SSH2();
