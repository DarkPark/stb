...
function webpack ( err, stats ) {
	...
	if ( err ) {
		...
		gutil.log(title, 'time:',    json.time.bold);
		...
		gutil.log(title, '\tNo\tID\tSize\tErrs\tWarns\tName'.grey);
		...
		json.modules.forEach(function ( module, index ) {
			gutil.log(title, '\t' +
				...
				(module.name.indexOf('(webpack)') === 0 || module.name[0] === '/' ? module.name.grey : module.name.replace(/\//g, '/'.grey))
			);
		});
